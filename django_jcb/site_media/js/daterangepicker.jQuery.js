...
jQuery.fn.daterangepicker = function(settings){
	...
	jQuery(this).bind('keydown',function(e){
		var key=e.which;
		if(key==38)jQuery('.ui-daterangepicker-next').trigger('click');
		else if(key==40)jQuery('.ui-daterangepicker-prev').trigger('click');
	});
	//defaults
	var options = jQuery.extend({
		presetRanges: [
			{text: '今天', dateStart: 'today', dateEnd: 'today' },
			{text: '前7天', dateStart: 'today-7days', dateEnd: 'today' },
			{text: '本月至今', dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },
			{text: '本年至今', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
			{text: '上月', dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  } }
	...
	if(options.arrows && rangeInput.size()==1){
		...
		.parent().find('a').click(function(){
			var nowText=jQuery('.ui-rangepicker-input').val().replace(' ','');
			var dateA;
			var dateB;
			/////////////////////////
			function parseTime(textTime)
			{
				...
				if(textTime.length!=1&&textTime.length!=2)return 0;
				if(textTime.length==1)
				{
					var returnValue=Date.parse(textTime[0].substr(0,4)+'/'+textTime[0].substr(4,2)+'/'+textTime[0].substr(6));
					if(returnValue==null)return 0;
					else return [returnValue,returnValue];
				}
				else
				{
					var returnValue1=Date.parse(textTime[0].substr(0,4)+'/'+textTime[0].substr(4,2)+'/'+textTime[0].substr(6));
					var returnValue2=Date.parse(textTime[1].substr(0,4)+'/'+textTime[1].substr(4,2)+'/'+textTime[1].substr(6));
					if(returnValue1==null||returnValue2==null)return 0;
					else return [returnValue1,returnValue2];
				}
			}
			////////////////////////////
			var time=parseTime(nowText);
			if(nowText==''||time==0)
			{
				dateA = rpPickers.find('.range-start').datepicker('getDate');
				dateB = rpPickers.find('.range-end').datepicker('getDate');
			}
			else
			{
				dateA=time[0];
				dateB=(time.length==1)?dateA:time[1];
				rpPickers.find('.range-start').datepicker('setDate', dateA);
				rpPickers.find('.range-end').datepicker('setDate', dateB);
			}
			/////////////////////////
